...
export mesh_to_delete=$(oci service-mesh mesh list --compartment-id ${mesh_compartment} --all | jq '.data.items[] | select(."lifecycle-state" == "ACTIVE") | ."display-name"' | tr -d '"')
export mesh_to_delete_id=$(oci service-mesh mesh list --compartment-id ${mesh_compartment} --all | jq '.data.items[] | select(."lifecycle-state" == "ACTIVE") | .id' | tr -d '"')
